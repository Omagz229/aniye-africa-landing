...
| **EX-M8** | Any admin | `PolicyLibrary.tsx`, `PeopleImport.tsx` | Tables rely on `overflow-x-auto`. Horizontal scrolling inside a page violates Doctrine §2.8, acceptable only as a fallback | Card layouts below `lg`, as `PeopleDirectory` already does | S | EX-C2 | Open | ✅ **Resolved 2026-07-29.** `PolicyLibrary.tsx` and `PeopleImport.tsx` — the two components this finding named — **no longer contain tables at all**, so the `overflow-x-auto` it described is gone. `PeopleDirectory` was cited here as the *good example* and still is: it renders cards below `lg` and a table above. A separate desktop overflow discovered on the same pass (the H3.2 address badge widening the Added column by 94px) was fixed by stacking the badges. |

...
> by ADR-014: `id`, `workspaceId`, `momentId`, `personId`, `fulfilmentId`, `recognitionOrderId`,
...
than to Operations conceptually — Moment currently belongs to the **Relationship Engine**. Storing a
Knowledge-domain record in the same document as a Relationship-Engine-domain record does not transfer
either record's conceptual ownership; it reflects only that no backend exists yet to split them
(ADR-010 remains the gate on that split before any external pilot).
...
*System Atlas v3.17 — Aniyé Africa — August 2026*
*Maintained alongside the codebase. Update this document whenever platform direction changes.*
*v3.17: **H3.8 reconciliation — a second, independent implementation of ADR-014 converged with the one v3.16 documents.** A structurally different build of H3.8 was completed in parallel and reconciled against `origin/recovery/h3-reconstruction` commit `a7fd52aeacc188e463676f07826d5917a704844b` — the commit v3.16 below documents about itself, preserved here unchanged as published history. Both implementations agree byte-for-byte on every fact ADR-014 governs: `MOMENT_STATUSES`'s fourth value `Closed`, the eleven-field `Memory`, the six-key `MomentClosed` payload, the nine-field timeline whitelist, `OperationsState` **v9**, and that closure writes no Decision. They diverge only in internal structure, and this reconciliation keeps this build's choices where they survive rerun: `lib/operations/timeline.ts` stays a module separate from `lib/operations/closure.ts`, and `validate:timeline` (**12 checks**) stays its own suite rather than merging into `validate:closure` (**51 checks** here, against 48 merged on the other build). Rerun on this reconciled tree — not restated from either build's own prior claim — typecheck is clean, lint holds the existing baseline of **47 problems (26 errors, 21 warnings)** with no new regression, the build succeeds at **34 routes**, and all fifteen suites pass: **614 checks** (551 pre-H3.8 + 51 `validate:closure` + 12 `validate:timeline`), against the other build's own **599 across fourteen**. Verified live in a browser on this tree: an existing `Closed` moment's detail page renders the terminal Memory panel, the irreversible-closure notice, the `Closed` badge and filter, and ten Decisions with no closure Decision among them; its relationship timeline page renders the nine-field safe projection with no cost, vendor, courier or QA detail reaching it. Component names differ from the other build's own naming (`ClosurePanel.tsx` / `RecipientTimeline.tsx` here vs `CloseMomentPanel.tsx` / `RelationshipTimeline.tsx` there) — a naming divergence, not an error in either. `docs/adr/README.md` is kept exactly as the other build left it, correcting this build's own gap of never having updated that registry. Milestone count and H3/H4 status are unchanged from v3.16: **26 of 37**, H3 **8 of 8**, H4.0 next.*

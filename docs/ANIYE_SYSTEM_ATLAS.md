...
| **ADR-013** | Commercial role, pilot currency and the `RecognitionOrder` financial model | Accepted 2026-07-30 | ⬜ **No** — H3.7 has not begun. Resolves **CP-U3 / OPS-U3** (`commercialRole = MerchantOfRecord`, a platform and pilot posture rather than a legal opinion) and **CP-U4** (**NGN**). FX stays out of H3.7; `estimatedCustomerCharge` is a **manual per-order quotation**; `estimatedItemCost` is superseded by `estimatedVendorCost`; `grossMargin` stays derived and never stored |
...
*System Atlas v3.13 — Aniyé Africa — July 2026*

...
| **`OperationsState` schema** | **v9** — additive `memories` rung; versioned independently (ADR-010) |
| **Last completed milestone** | **H3.8** — Confirmation + Memory. **[ADR-014](adr/ADR-014-moment-closure-memory-and-safe-timeline.md) implemented**: atomic closure, immutable Memory and customer-safe Operations timeline |
| **Next milestone** | **H4.0** — Pre-pilot completion |
| **Milestones** | **26 of 37 complete** — see *Milestone count* |
| **H3 exit** | ✅ **Complete, 8 of 8.** The closed operational loop ends at closure and the safe relationship timeline |
| **Outstanding** | ⚠️ **Live responsive testing on real devices** — still never performed. Browser verification at emulated widths is not a substitute; it is a pre-pilot **[P]** item (H4.0). ⚠️ **H3.6 and H3.8 shipped with no browser run** — their new operator surfaces are covered by automated checks and static review only. **H3.7 was verified live** at 1440px, 768px and a browser-clamped 500px |
...
| **H3.8** | **Confirmation + Memory** — the Moment closes; an immutable Memory enters the relationship timeline through an exact customer-safe projection. Governed by [ADR-014](adr/ADR-014-moment-closure-memory-and-safe-timeline.md) | 10 | H3.7 · ADR-014 | **[R]** | ✅ **Complete** |
...
| H3.8 | A closed Moment appears in the Operations **safe recipient timeline projection** with occasion, date and outcome — and **no commercial detail**. Actual customer-facing visibility remains H4.0 |
...
**H3.7 is complete.** It introduced the RecognitionOrder and internal commercial tracking described
above, but still introduced no payment, cash-received or accounting claim.

### H3.8 — architecture governed and implemented

**[ADR-014](adr/ADR-014-moment-closure-memory-and-safe-timeline.md) accepted by Council, 2026-07-31.**
Its accepting commit was documentation only — no code, schema, migration, validation, route or UI
changed at that gate. It resolved the architecture later implemented by H3.8:
...
- Memory is governed to persist in `OperationsState` **v9** (additive `memories`, no backfill, no
  Moment rewritten), while **domain ownership stays with Knowledge** — the Relationship Engine owns
  the Moment's current state, Operations only orchestrates the atomic write.
...
**Implementation status, 2026-07-31.** H3.8 is complete. `OperationsState` is **v9** and Workspace
stays **v7**. The only new Moment transition is `ReadyForExecution → Closed`; one confirmation writes
the terminal state, one immutable Memory and one `MomentClosed` Event atomically, with no Decision.
The Operations-only `/operations/timeline/[personId]` route renders the exact nine-field whitelist;
H4.0 still owns customer-facing visibility. H3 is **8 of 8**, the milestone count is **26 of 37**,
and H4.0 is next.
...
*Master Roadmap v2.4 — Aniyé Africa — 31 July 2026*
*v2.4: **H3.8 — Confirmation + Memory implemented.** [ADR-014](adr/ADR-014-moment-closure-memory-and-safe-timeline.md) is implemented without variance. `OperationsState` **v8 → v9** adds only `memories: []`; no Memory is backfilled and no existing Moment is rewritten. `Closed` is the sole new terminal Moment status and may be entered only through the atomic closure operation from `ReadyForExecution`; it cannot reopen or cancel. Closure rereads and rebuilds authority, requires delivered Fulfilment, reconciled RecognitionOrder and `ProofReceived` where the frozen snapshot requires it, then writes Moment + immutable eleven-field Memory + exact `MomentClosed` Event in one storage write and no Decision. Legacy Fulfilments without orders remain honestly unclosable. The Operations-only relationship timeline is an exact nine-field whitelist with gift category resolved through the order's immutable item-selection reference; commercial, partner, address, contact, proof and internal-note fields are excluded. Two routes added: `/operations/moments/[id]/close` and `/operations/timeline/[personId]`; no top-level navigation and no customer-facing route. New `validate:closure` suite adds **48 checks**; all **599 checks across fourteen suites** pass. Typecheck and build pass; lint remains at the pre-existing **47 problems (26 errors, 21 warnings)**; build contains **34 routes**. Workspace stays **v7**; `OperationsState` is **v9**; H3 is **8 of 8**; milestone count is **26 of 37**; **H4.0 is next**. No tag was created or moved.*

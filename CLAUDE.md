...
- **The next milestone is H3.7 — Recognition Order and commercial tracking. Its governance is resolved ([ADR-013](docs/adr/ADR-013-commercial-role-pilot-currency-and-recognition-order.md), 2026-07-30) and it has not begun.** **CP-U3 / OPS-U3 and CP-U4 are closed.** `commercialRole = MerchantOfRecord` — Aniyé contracts with the corporate customer for the complete outcome and procures vendor and courier fulfilment as its own cost; this is a platform and pilot posture, **not a legal opinion**, and professional confirmation is required before any external pilot. **Every amount on a pilot `RecognitionOrder` is NGN**; a non-NGN supplier cost puts the order outside the approved architecture — never convert, default or compare it implicitly. **FX stays out of H3.7 entirely.** `estimatedCustomerCharge` is a **manual per-order quotation** — no formula, no percentage, no rate card, no pricing engine, and no payment or cash-received assumption. `estimatedItemCost` is superseded by `estimatedVendorCost` from the confirmed `VendorSelection` Decision. `grossMargin` stays derived on read and never stored. Vendor commission, sender service fee, corporate subscription, payment margin and featured placement are **deferred and non-authoritative** — subscription is account-level and can never be a `RecognitionOrder` field. First pilot: one corporate organization, Lagos fulfilment, city-first; **Nigeria → Cameroon remains the first cross-border corridor, sequenced after it, and is not abandoned.**
...
baselines are **504 checks across twelve suites**, lint **47 problems (26 errors, 21 warnings)**, and
a build of **30 routes**. Any milestone that changes them must say so and give the new figures.

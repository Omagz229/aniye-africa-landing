...
| ADRs 004–014 and registry | `docs/adr/` |
...
- **H3.8 — Confirmation + Memory is complete and [ADR-014](docs/adr/ADR-014-moment-closure-memory-and-safe-timeline.md) is implemented.** `OperationsState` is **v9** with an additive `memories` collection; Workspace remains **v7**. One confirmed operation re-reads current authority and atomically changes a `ReadyForExecution` Moment to irreversible `Closed`, creates exactly one immutable eleven-field `Memory`, and appends exactly one `MomentClosed` Event with no Decision. The closure gate requires a delivered Fulfilment, reconciled RecognitionOrder and proof when the frozen policy snapshot requires it; legacy delivered Fulfilments without orders cannot close. The Operations-only `/operations/moments/[id]/close` and `/operations/timeline/[personId]` routes expose the confirmation flow and an exact nine-field safe projection whose required `giftCategory` comes through the order's immutable `itemSelectionDecisionId`. Domain ownership remains separated: the Relationship Engine owns Moment current state, Knowledge owns Memory and `MomentClosed`, Operations orchestrates, and the shared client-side state is only the prototype persistence envelope. Recipient acknowledgement, customer-facing visibility, correction, production persistence and the checkpoint's unsupported Program-envelope consequence remain out of scope. **H3 is 8 of 8, 26 of 37 milestones are complete, and H4.0 — Pre-pilot completion is next.**
...
- **Structural persistence changes require** an ordered migration, a schema-version update, and a Recovery Ledger entry. The workspace chain is in `lib/migrations.ts` (currently v7) and moves one version at a time; `OperationsState` versions independently in `lib/operations/types.ts` (currently **v9**, the additive `memories` rung after v8's `recognitionOrders`). The two counters are independent and have differed and coincided at different times — never assert independence by asserting the numbers differ. Historical migrations are frozen — pin their value sets rather than referencing live enums.
...
**All fourteen suites, or the run is incomplete.** `validate:briefs` was previously missing from this
list, so following it as written under-ran the gates by a full suite (47 checks). The current
baselines are **599 checks across fourteen suites**, lint **47 problems (26 errors, 21 warnings)**, and

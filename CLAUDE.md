...
- **The next milestone is H4.0 — Pre-pilot completion.** H3 is now **complete at 8 of 8** — H3.8 landed. See the H3.8 bullet below for what it built. H4.0 is the checkpoint's **[P]** set: Recurring and Triggered Program modes, customer-facing Moment visibility, approval flows where required, delivery-failure/redelivery handling, **EX-H1** assessment autosave, **EX-H3** `PolicyForm` rebuild, live responsive testing on real devices, and a backend with real tenant isolation and access control.
- **H3.8 — Confirmation + Memory is complete and [ADR-014](docs/adr/ADR-014-moment-closure-memory-and-safe-timeline.md) is implemented.** **Confirmation means an operator confirming a delivered, reconciled recognition is complete** — distinct from `Delivered`, `ProofReceived` and RecognitionOrder reconciliation. **Recipient acknowledgement is unsupported and excluded**, not architecturally impossible. The terminal Moment status is **`Closed`**, superseding the checkpoint's `Fulfilled`, entered only from `ReadyForExecution`, and **irreversible** — no reopen, no re-cancellation; `commitMomentClosure` is the only path in, and no caller may assert the terminal status directly. **Domain ownership does not follow storage location:** the Relationship Engine owns the Moment's current state, Knowledge owns Memory and `MomentClosed`, Operations only orchestrates the atomic write, and `OperationsState` is the shared prototype persistence envelope, not a domain boundary. The canonical **Memory has exactly eleven fields** (`id`, `workspaceId`, `momentId`, `personId`, `fulfilmentId`, `recognitionOrderId`, `outcome`, `outcomeDate`, `createdByActorType`, optional `createdByActorId`, `createdAt`), superseding the Atlas §4 draft in full — occasion, target date, recipient identity and gift category are **referenced, never copied**; `outcomeDate` comes from the authoritative `Delivered` Event, never the closure instant. The safe recipient timeline is a **nine-field exact-key whitelist projection** (never a spread with fields hidden in the UI), with gift category required and resolved through the RecognitionOrder's immutable `itemSelectionDecisionId`, never through whichever `ItemSelection` happens to be live. **Three separate prohibition boundaries** govern persisted Memory, the `MomentClosed` Event payload and the projection — each has its own permitted and forbidden keys; do not collapse them into one list. Closure appends one `MomentClosed` Event and **records no Decision** — ADR-006's own judgement test, already applied by ADR-012 to dispatch and delivery, applies the same way here. A pre-H3.7 delivered Fulfilment without a RecognitionOrder **can never close**; no order or commercial history is invented for it. The checkpoint's *"envelope consumption final"* is **unsupported by the current architecture** and is not implemented. **H3.8 is Operations-only** — the customer-facing timeline route stays H4.0's. `OperationsState` is now **v9** (additive `memories`); Workspace stays **v7**.
...
npm run validate:orders          npm run validate:closure
npm run validate:timeline
```

**All fifteen suites, or the run is incomplete.** `validate:briefs` was previously missing from this
list, so following it as written under-ran the gates by a full suite (47 checks). The current
baselines are **614 checks across fifteen suites**, lint **47 problems (26 errors, 21 warnings)**, and
a build of **34 routes**. Any milestone that changes them must say so and give the new figures.

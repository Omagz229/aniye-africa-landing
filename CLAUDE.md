...
- **The next milestone is H3.8 — Confirmation + Memory. [ADR-014](docs/adr/ADR-014-moment-closure-memory-and-safe-timeline.md) governs its architecture (Council, 2026-07-31) and it has not begun.** No `Memory` type or collection, no `Closed` Moment status, no `MomentClosed` Event, no closure repository operation and no recipient-timeline route exist. `OperationsState` stays **v8** and Workspace stays **v7** until an implementation milestone lands the additive v9 `memories` rung. **Confirmation means an operator confirming a delivered, reconciled recognition is complete** — distinct from `Delivered`, `ProofReceived` and RecognitionOrder reconciliation. **Recipient acknowledgement is unsupported and excluded**, not architecturally impossible. The terminal Moment status is **`Closed`**, superseding the checkpoint's `Fulfilled`, entered only from `ReadyForExecution`, and **irreversible** — no reopen, no re-cancellation. **Domain ownership does not follow storage location:** the Relationship Engine owns the Moment's current state, Knowledge owns Memory and `MomentClosed`, Operations only orchestrates the atomic write, and `OperationsState` is the shared prototype persistence envelope, not a domain boundary. The canonical **Memory has exactly eleven fields** (`id`, `workspaceId`, `momentId`, `personId`, `fulfilmentId`, `recognitionOrderId`, `outcome`, `outcomeDate`, `createdByActorType`, optional `createdByActorId`, `createdAt`), superseding the Atlas §4 draft in full — occasion, target date, recipient identity and gift category are **referenced, never copied**. The safe recipient timeline is a **nine-field exact-key whitelist projection** (never a spread with fields hidden in the UI), with gift category required and resolved through the RecognitionOrder's immutable `itemSelectionDecisionId`, never through whichever `ItemSelection` happens to be live. **Three separate prohibition boundaries** govern persisted Memory, the `MomentClosed` Event payload and the projection — each has its own permitted and forbidden keys; do not collapse them into one list. Closure appends one `MomentClosed` Event and **records no Decision** — ADR-006's own judgement test, already applied by ADR-012 to dispatch and delivery, applies the same way here. A pre-H3.7 delivered Fulfilment without a RecognitionOrder **can never close**; no order or commercial history is invented for it. The checkpoint's *"envelope consumption final"* is **unsupported by the current architecture** and is not implemented. **H3.8 remains Operations-only** — the customer-facing timeline route stays H4.0's.
